...
```sh
curl -fsSL https://www.fftw.org/fftw-2.1.5.tar.gz | tar xz
cd fftw-2.1.5
patch -p1 < ../patches/05_ac_define_syntax.diff
autoreconf -fvi
./configure --disable-fortran
make -j"$(sysctl -n hw.ncpu)"
```

[patches/05_ac_define_syntax.diff](patches/05_ac_define_syntax.diff)
is vendored from
[sources.debian.org/patches/fftw/2.1.5-6/](https://sources.debian.org/patches/fftw/2.1.5-6/).

## CI

[.github/workflows/build.yml](.github/workflows/build.yml) runs the
above on `macos-14` and `macos-15` runners (both Apple Silicon) for
single- and double-precision builds and executes `fftw_test`.
